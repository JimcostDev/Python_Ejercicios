...
def obtener_minimo_comun_multiplo(numeros):
    ...
    for factor in factores_comunes:
        max_conteo = 0
        for num in numeros:
            conteo = conteo_factores[num].get(factor, 0) #por columna
...
if __name__ == "__main__":
    ...
    numeros = []
    cantidad = int(input('''¿A cuantos numeros vamos a calcular su mcm y MCD (cantidad)?, digita un valor entre 2 y 10 => '''))
    # validar que la cantidad sea >= 2 y <= 10
    if cantidad >= 2 and cantidad <= 10:
        for x in range(cantidad):
            x = int(input(f'Ingresa valor de número {x+1}: '))
        numeros.append(x)  # agregar numeros a la lista
    else:
